...
import { MakePlaylist } from "@/store/PlatlistSlice";
...
export default function Cardstackk({ playlists }: playlistProps) {
  ...
  const handleOnClick = () => {
    dispatch(MakePlaylist(playlists))
    navigate(`/watch/${playlists.videos[0]._id}`)
  }

  return (
    <div className="h-full flex items-center justify-center md:justify-start pt-4 pb-6 w-full mt-4 cursor-pointer" onClick={handleOnClick}>
