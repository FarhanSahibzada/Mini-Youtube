...
function UploadVideo() {
    ...
    const onSubmitt = async (data: FieldValues) => {
        ...
        try {
            const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/videos/upload-video`,
            ...
            if (response && response.data) {
                ...
                if (selectedPlaylists.length > 0) {
                    try {
                        await axios.patch(
                            `${import.meta.env.VITE_BACKEND_URL}/api/v1/playlist/add-video/${selectedPlaylists[0]}/${_id}`,
                            {},
                        ...
                        setLoading(false)
                        console.log("error on adding playlist", error);
                    }
    ...
    if (!loading) {
        ...
        return (
            <div className="flex flex-col w-full items-center justify-center min-h-screen bg-gradient-to-r from-slate-200 to-base-300 backdrop-blur-lg bg-white/30">
